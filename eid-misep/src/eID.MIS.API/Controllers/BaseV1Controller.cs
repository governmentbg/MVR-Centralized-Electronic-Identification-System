...
public class BaseV1Controller : ControllerBase
{
    ...
    protected string? GetSystemName() =>
        HttpContext.User.Claims.FirstOrDefault(d => string.Equals(d.Type, Claims.SystemName, StringComparison.InvariantCultureIgnoreCase))?.Value;
    ...
    protected SortedDictionary<string, object> BeginAuditLog(LogEventCode logEventCode, string? targetUserId = default, params (string Key, object Value)[] additionalEventPayloadData)
        => BeginAuditLog(logEventCode, null, targetUserId, additionalEventPayloadData);

    protected SortedDictionary<string, object> BeginAuditLog(LogEventCode logEventCode, object? request, string? targetUserId = default, params (string Key, object Value)[] additionalEventPayloadData)
    {
