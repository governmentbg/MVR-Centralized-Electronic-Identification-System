# eID.Signing solution
Целта на този solution е да предостави основа, която да служи като start-up template.

# Структура на solution-a
1. eID.Signing.API (.net6 app) - API Gateway. Получава и валидира HTTP заявки. Подготвя съответните команди за Application-а и ги изпраща през RabbitMQ  
2. eID.Signing.API.Public (.net6 app) - API Gateway предназначен за ПГ и МПГ. Получава и валидира HTTP заявки. Подготвя съответните команди за Application-а и ги изпраща през RabbitMQ  
3. eID.Signing.Application (.net6 app) - Инстанцира service-ите от eID.Signing.Service. Предвиден да scale-ва при натоварвания.  
4. eID.Signing.Service (class library) - Бизнес логика и връзки с БД  
4.1. Unit Tests  
5. eID.Signing.Contracts (class library)  - Контракти, които се използват при пренос на данни (команди, event-и, резултати)
...
Тези, които ще се интересуват от някои от event-ите ще трябва да си създадат temporary (препоръчително) queue под exchange-а eID.Signing.Contracts.Events:T
...
## Database
`DbContext` се намира в проекта `eID.Signing.Service` За да се направи миграция е нужно да се намирате тук: `.\eID.Signing.Service>`
...
1. Стартирай `eID.Signing.API` и `eID.Signing.Application`
2. Отвари [Swagger](http://localhost:60011/swagger/index.html)
...
# UI стартиране
1. В конзола стигате до папката на UI `..\src\eID.Signing.UI`
2. Изпълнявате:
  - `npm install`
  - `npm start`
...
# URLs
[API URL](http://localhost:60011/api/v1) _http://localhost:60011/api/v1_  
[Swagger URL](http://localhost:60011/swagger) _http://localhost:60011/swagger_  
[API Health check](http://localhost:60011/health/ready) _http://localhost:60011/health/ready_  
[Application metrics URL](http://localhost:60012/metrics) _http://localhost:60012/metrics_  
[Application Health check](http://localhost:60012/health/ready)
