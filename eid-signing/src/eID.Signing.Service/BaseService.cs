...
public class BaseService
{
    ...
    protected static ServiceResult<T> BadRequest<T>(string errorMessage, string errorData)
    {
        return new ServiceResult<T>
        {
            StatusCode = HttpStatusCode.BadRequest,
            Errors = new KeyValuePair<string, string>[] { new KeyValuePair<string, string>(errorMessage, errorData) }.ToList()
        };
    }

    protected static ServiceResult<T> BadRequest<T>()
    {
        return new ServiceResult<T> { StatusCode = HttpStatusCode.BadRequest, Error = "Bad Request" };
    }
