...
import { IEmpowerment, IHidePreviewEventEmitter } from '../../interfaces/authorization-register.interfaces';
import {
...
export class EmpowermentPreviewComponent implements OnChanges, OnDestroy, OnInit {
    constructor(
    ...
    getSignedDateByUid(uid: any) {
        const signedDate = this.empowerment.empowermentSignatures.find(s => s.signerUid === uid)?.dateTime;
        if (signedDate) {
            return this.translocoLocaleService.localizeDate(signedDate, this.translocoLocaleService.getLocale(), {
                timeStyle: 'medium',
            });
        }
        return;
