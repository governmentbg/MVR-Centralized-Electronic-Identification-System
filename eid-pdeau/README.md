...
1. eID.PDEAU.API (.net6 app) - API Gateway. Получава и валидира HTTP заявки. Подготвя съответните команди за Application-а и ги изпраща през RabbitMQ  
2. eID.PDEAU.Application (.net6 app) - Инстанцира service-ите от eID.PDEAU.Service. Предвиден да scale-ва при натоварвания.  
3. eID.PDEAU.Service (class library) - Бизнес логика и връзки с БД  
3.1. Unit Tests  
4. eID.PDEAU.Contracts (class library)  - Контракти, които се използват при пренос на данни (команди, event-и, резултати)
...
Тези, които ще се интересуват от някои от event-ите ще трябва да си създадат temporary (препоръчително) queue под exchange-а eID.PDEAU.Contracts.Events:T
...
## Database
`DbContext` се намира в проекта `eID.PDEAU.Service` За да се направи миграция е нужно да се намирате тук: `.\eID.PDEAU.Service>`
...
1. Стартирай `eID.PDEAU.API` и `eID.PDEAU.Application`
2. Отвори [Swagger](http://localhost:60019/swagger/index.html)
...
# UI стартиране
1. В конзола стигате до папката на UI `..\src\eID.PDEAU.UI`
2. Изпълнявате:
  - `npm install`
  - `npm start`
...
# URLs
[API URL](http://localhost:60019/api/v1) _http://localhost:60019/api/v1_  
[API failing request](http://localhost:60019/api/v1/People?firstName=bad&lastName=bad) _http://localhost:60019/api/v1/People?firstName=bad&lastName=bad_  
[Swagger URL](http://localhost:60019/swagger) _http://localhost:60019/swagger_  
[API Health check](http://localhost:60019/health/ready) _http://localhost:60019/health/ready_  
[Application metrics URL](http://localhost:60020/metrics) _http://localhost:60020/metrics_  
[Application Health check](http://localhost:60020/health/ready)
...
# Описание на APIs
1. eID.PDEAU.API - за админинстраторите от МВР
1. eID.PDEAU.API.Public - за граждани
1. eID.PDEAU.API.Public.Admin - за администратора на ДЕАУ