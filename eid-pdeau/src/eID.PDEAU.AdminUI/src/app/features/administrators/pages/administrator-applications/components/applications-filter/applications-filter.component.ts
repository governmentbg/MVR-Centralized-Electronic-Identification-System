...
import { statuses } from '@app/features/administrators/components/application-status/application-status.component';
            ...
            this.statuses = Object.entries(statuses).map(([key, { translationKey }]) => {
                return {
                    name: this.translateService.translate(
                        `modules.administrators.applications.statuses.${translationKey}`
                    ),
                    code: key,
                };
            });
            this.applicationTypes = [
                {
                    ...
                    name: this.translateService.translate(
                        `modules.administrators.applications.types.${ApplicationType.REGISTER}`
                    ),
                },
                {
                    id: ApplicationType.RESUME,
                    name: this.translateService.translate(
                        `modules.administrators.applications.types.${ApplicationType.RESUME}`
                    ),
                },
                {
                    id: ApplicationType.STOP,
                    name: this.translateService.translate(
                        `modules.administrators.applications.types.${ApplicationType.STOP}`
                    ),
                },
                {
                    id: ApplicationType.REVOKE,
                    name: this.translateService.translate(
                        `modules.administrators.applications.types.${ApplicationType.REVOKE}`
                    ),
                },
            ];
        });
        ...
        Object.entries(this.filterForm.getRawValue()).forEach((formValue: any) => {
            ...
            if (this.performDataTypeCheck(formValue[1])) {
                const preparedObject = {
                    key: formValue[0],
                    value: formValue[1],
                    label: `modules.administrators.applications.filter.${formValue[0]}`,
                };
