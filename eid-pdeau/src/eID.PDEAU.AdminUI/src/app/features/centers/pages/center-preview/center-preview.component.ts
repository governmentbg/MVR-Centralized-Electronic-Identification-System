...
import { CircumstanceStatus, eidManagerStatus } from '@app/features/administrators/enums/administrators.enum';
...
export class CenterPreviewComponent implements OnInit {
    constructor(
        ...
        const langChanges$ = this.translateService.langChanges$.subscribe(() => {
            this.breadcrumbItems = [
                {
                    label: this.translateService.translate('modules.centers.registrations.title'),
                    onClick: () => this.router.navigate(['/centers/registrations']),
                },
                {
                    label: this.translateService.translate('modules.centers.registrations.preview'),
                },
            ];
            this.roles = [
                {
    ...
    detailsQuery = new RequestHandler({
        requestFunction: this.centersService.fetchCenterDetails,
        onSuccess: data => {
    ...
    ngOnInit(): void {
        ...
        if (!applicationId) {
            this.router.navigate(['/centers/registrations']);
            return;
        }
    ...
    get translationKey() {
        switch (this.action) {
            ...
            case CircumstanceStatus.APPROVED:
                return 'modules.centers.circumstances.active-dialog';
            default:
                return '';
        }
    }
    ...
    get titleTranslation() {
        switch (this.action) {
            ...
            case CircumstanceStatus.APPROVED:
                return this.translateService.translate('modules.centers.circumstances.active-dialog.txtTitle');
            default:
                return '';
        }
    }
    ...
    get descriptionTranslation() {
        switch (this.action) {
            ...
            case CircumstanceStatus.APPROVED:
                return this.translateService.translate('modules.centers.circumstances.active-dialog.txtDescription');
            default:
                return '';
        }
    }
