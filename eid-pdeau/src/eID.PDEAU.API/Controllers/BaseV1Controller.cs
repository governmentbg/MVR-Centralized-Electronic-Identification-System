...
[RoleAuthorization(UserRoles.AppAndCisAdmins, UserRoles.PLSAdministrator)]
[SetRequestId]
public class BaseV1Controller : ControllerBase
{
    ...
    {
        //TODO: use Cyrillic names when token is properly configured
        var givenName = HttpContext.User.Claims.FirstOrDefault(d => d.Type == Claims.GivenName || d.Type.Contains("givenname"))?.Value;
        var middleName = HttpContext.User.Claims.FirstOrDefault(d => d.Type == Claims.MiddleName || d.Type.Contains("middlename"))?.Value;
        var familyName = HttpContext.User.Claims.FirstOrDefault(d => d.Type == Claims.FamilyName || d.Type.Contains("surname"))?.Value;
    ...
    /// <summary>
    /// Checks whether the user is in the Private Law Subject (PLS) role.
    /// </summary>
    /// <returns>True if the user is in the role; otherwise, false.</returns>
    protected bool IsUserInPLSRole()
    {
        var user = HttpContext.User;

        return user.Claims.Where(c => c.Type == ClaimTypes.Role).Any(c => c.Value == UserRoles.PLSAdministrator);
    }
}
