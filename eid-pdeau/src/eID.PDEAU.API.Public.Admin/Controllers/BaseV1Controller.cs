...
public class BaseV1Controller : ControllerBase
{
    ...
    /// <summary>
    /// Provider Id. Mandatory property. It comes from token
    /// </summary>
    ...
    protected string GetProviderId() => ProviderId.ToString();
        ...
        SortedDictionary<string, object>? payload = default)
    {
        ...
        // We add explicit ProviderId
        payload ??= new SortedDictionary<string, object>();
        payload[nameof(ProviderId)] = ProviderId;
...
public class RequireSystemIdAttribute : ActionFilterAttribute
{
    public override void OnActionExecuting(ActionExecutingContext context)
    {
        var providerId = context.HttpContext.User.Claims.FirstOrDefault(d => d.Type == Claims.SystemId)?.Value;
        ...
        Guid parsedProviderId = Guid.Empty;
        if (providerId is null)
        {
            validationProblemDetails.Errors.Add(Claims.SystemId, new[] { $"{Claims.SystemId} missing from token" });
        }
        else if (!Guid.TryParse(providerId, out parsedProviderId))
        {
            validationProblemDetails.Errors.Add(Claims.SystemId, new[] { $"Invalid {Claims.SystemId} in token." });
        }
        ...
        if (validationProblemDetails.Errors.Any())
        {
            ...
            validationProblemDetails.Extensions["traceId"] = requestId;
            context.Result = new BadRequestObjectResult(validationProblemDetails);
        }
        else
        {
            var controller = (BaseV1Controller)context.Controller;
            controller.ProviderId = parsedProviderId;
        }
