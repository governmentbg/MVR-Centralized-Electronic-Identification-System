...
import bg.bulsi.mvr.mpozei.gateway.api.v1.NomenclatureApiDelegate;
...
public class NomenclatureApiDelegateService implements NomenclatureApiDelegate {
    ...
    @Override
    public Mono<List<ApplicationStatus>> getAllApplicationStatuses(ServerWebExchange exchange) {
        return Mono.just(Arrays.stream(ApplicationStatus.values()).toList());
    }

    @Override
    public Mono<List<CertificateStatus>> getAllCertificateStatuses(ServerWebExchange exchange) {
        return Mono.just(Arrays.stream(CertificateStatus.values()).toList());
    }
}
