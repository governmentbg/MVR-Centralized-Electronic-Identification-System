...
public class Startup
{
    ...
    public void ConfigureServices(IServiceCollection services)
    {
        ...
        services.AddSwaggerGen(options =>
        {
            ...
            {
                Title = "eID - PIVR HTTP API",
                Version = "v1",
                Description = "Подсистема за интеграция с външни регистри (ПИВР)"
            });
        ...
        var thirdPartyRequestTypes = new[]
        {
            typeof(Contracts.Commands.GetDeceasedByPeriod),
            typeof(Contracts.Commands.CheckUidRestrictions),
            typeof(Contracts.Commands.GetDateOfProhibition),
            typeof(Contracts.Commands.GetDateOfDeath),
            typeof(Contracts.Commands.GetIdChanges),
            typeof(Contracts.Commands.GetStatutChanges),
            typeof(Contracts.Commands.MVRGetForeignIdentityV2),
            typeof(Contracts.Commands.MVRGetPersonalIdentityV2),
            typeof(Contracts.Commands.RegiXSearchCommand),
            typeof(Contracts.Commands.VerifySignature),
        };
        services.AddMassTransit(mt =>
        {
            ...
            // There is no longer a way to set default timeout for all request clients
            foreach (var type in thirdPartyRequestTypes)
            {
                mt.AddRequestClient(type, TimeSpan.FromSeconds(60));
            }
        });
        ...
        services.AddScoped<IApiUsageTrackerService, ApiUsageTrackerService>();
            ...
            opts.OnBeforeWriteDetails = (ctx, details) =>
            {
                    ...
                    && ctx.Items.ContainsKey("Payload"))
                {
                    ...
                    _auditLogger.LogEvent(new AuditLogEvent
                    {
                        ...
                        RequesterUserId = ctx.User.Claims.FirstOrDefault(d => d.Type == Claims.CitizenIdentifier)?.Value ?? string.Empty,
