...
namespace eID.PIVR.Service.Extensions;

public static class DistributedCacheExtensions
{
    public static Task SetAsync<T>(this IDistributedCache cache, string key, T value,
        int expireAfterInHours = 0)
    {
        if (cache is null)
        {
        ...
        if (expireAfterInHours > 0)
        {
            expireAfter = TimeSpan.FromHours(expireAfterInHours);
            // Calculate time till midnight.
            var utcNow = DateTime.UtcNow;
            var timeToEndOfDay = utcNow.Date.AddDays(1).AddMilliseconds(-1) - utcNow;

            // If expiration time using configured duration is after today, set to end of day
            if (expireAfter > timeToEndOfDay)
            {
                expireAfter = timeToEndOfDay;
            }
        }
        options.AbsoluteExpirationRelativeToNow = expireAfter;
