# eID.PIVR solution
ПИВР - Подсистема за интеграция с външни регистри 

# Структура на solution-a
1. eID.PIVR.API (.net6 app) - API Gateway. Получава и валидира HTTP заявки. Подготвя съответните команди за Application-а и ги изпраща през RabbitMQ  
2. eID.PIVR.Application (.net6 app) - Инстанцира service-ите от eID.PIVR.Service. Предвиден да scale-ва при натоварвания.  
3. eID.PIVR.Service (class library) - Бизнес логика и връзки с БД  
3.1. Unit Tests  
4. eID.PIVR.Contracts (class library)  - Контракти, които се използват при пренос на данни (команди, event-и, резултати)
...
Тези, които ще се интересуват от някои от event-ите ще трябва да си създадат temporary (препоръчително) queue под exchange-а eID.PIVR.Contracts.Events:T
...
## Database
`DbContext` се намира в проекта `eID.PIVR.Service` За да се направи миграция е нужно да се намирате тук: `.\eID.PIVR.Service>`
...
2. Отвори [Swagger](http://localhost:60009/swagger/index.html)
3. За справка в RegiX: `POST`**/api/v1/Registries/regix/search** `Request body` paste-ни
```json
{
...
# URLs
[API URL](http://localhost:60009/api/v1) _http://localhost:60009/api/v1_  
[Swagger URL](http://localhost:60009/swagger) _http://localhost:60009/swagger_  
[API Health check](http://localhost:60009/health/ready) _http://localhost:60009/health/ready_  
[Application metrics URL](http://localhost:60010/metrics) _http://localhost:60010/metrics_   
[Application Health check](http://localhost:60010/health/ready) _http://localhost:60010/health/ready_
...
# Примери за вмъкване на информация от МВР в таблиците
## Вмъкване в **IDChanges** таблицата
```SQL
INSERT INTO public."IDChanges"(
	"OldPersonalId", "OldUidType", "NewPersonalId", "NewUidType", "Date")
	VALUES ('2345678901', 2, '7211106259', 1, '2024-10-11');
```
## Вмъкване в **StatutChanges** таблицата
```SQL
INSERT INTO public."StatutChanges"(
	"PersonalId", "UidType", "Date")
	VALUES ('7311106259', 1, '2024-10-11');
```