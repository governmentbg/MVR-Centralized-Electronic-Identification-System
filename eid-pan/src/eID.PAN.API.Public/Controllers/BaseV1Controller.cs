...
{
    public const string UNKNOWN_SYSTEM = "Unknown system";
    private const string MISSING_PROVIDERID = "Missing ProviderId";
    ...
    public BaseV1Controller(ILogger logger, IConfiguration configuration, AuditLogger auditLogger)
    {
    ...
    protected string GetSystemName() =>
        HttpContext.User.Claims.FirstOrDefault(d => d.Type == Claims.SystemName)?.Value ?? "Unknown system";
    protected string GetSystemId() =>
        HttpContext.User.Claims.FirstOrDefault(d => d.Type == Claims.SystemId)?.Value ?? string.Empty;
        ...
        SortedDictionary<string, object>? payload = default)
    {
        ...
        _auditLogger.LogEvent(new AuditLogEvent
        {
            ...
            RequesterSystemId = GetSystemId() != UNKNOWN_SYSTEM ? GetSystemId() : "PAN",
            RequesterSystemName = GetSystemName(),
            RequesterUserId = !string.IsNullOrWhiteSpace(GetSystemId()) ? GetUid() : GetUserId(),
    ...
    protected SortedDictionary<string, object> BeginAuditLog(LogEventCode logEventCode, object? request, string? targetUserId, params (string Key, object Value)[] additionalEventPayloadData)
    {
        ...
        // Check if endpoint called from system or user
        var systemName = GetSystemName();
        if (systemName != UNKNOWN_SYSTEM)
        {
            // System
            eventPayload["RequesterSystemName"] = systemName;
            eventPayload["RequesterSystemId"] = GetSystemId();
        }
        else
        {
            // User
            eventPayload["RequesterSystemName"] = "PAN";
            eventPayload[AuditLoggingKeys.RequesterUid] = GetUid();
            eventPayload[AuditLoggingKeys.RequesterUidType] = GetUidType().ToString();
            eventPayload[AuditLoggingKeys.RequesterName] = GetUserFullName();
        }

        // If API has been called from DEAU
        var providerId = GetSystemId();
        if (providerId != MISSING_PROVIDERID)
        {
            eventPayload["ProviderId"] = providerId;
        }
