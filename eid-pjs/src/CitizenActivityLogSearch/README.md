...
1. eID.PJS.API (.net6 app) - API Gateway. Получава и валидира HTTP заявки. Подготвя съответните команди за Application-а и ги изпраща през RabbitMQ  
2. eID.PJS.Application (.net6 app) - Инстанцира service-ите от eID.PJS.Service. Предвиден да scale-ва при натоварвания.  
3. eID.PJS.Service (class library) - Бизнес логика и връзки с БД  
3.1. Unit Tests  
4. eID.PJS.Contracts (class library)  - Контракти, които се използват при пренос на данни (команди, event-и, резултати)
...
Тези, които ще се интересуват от някои от event-ите ще трябва да си създадат temporary (препоръчително) queue под exchange-а eID.PJS.Contracts.Events:T
...
## Database
`DbContext` се намира в проекта `eID.PJS.Service` За да се направи миграция е нужно да се намирате тук: `.\eID.PJS.Service>`
...
1. Стартирай `eID.PJS.API` и `eID.PJS.Application`
2. Отвари [Swagger](http://localhost:60013/swagger/index.html)
...
# UI стартиране
1. В конзола стигате до папката на UI `..\src\eID.PJS.UI`
2. Изпълнявате:
  - `npm install`
  - `npm start`
...
# URLs
[API URL](http://localhost:60013/api/v1) _http://localhost:60013/api/v1_  
[API failing request](http://localhost:60013/api/v1/People?firstName=bad&lastName=bad) _http://localhost:60013/api/v1/People?firstName=bad&lastName=bad_  
[Swagger URL](http://localhost:60013/swagger) _http://localhost:60013/swagger_  
[API Health check](http://localhost:60013/health/ready) _http://localhost:60013/health/ready_  
[API metrics URL](http://localhost:60013/metrics) _http://localhost:60013/metrics_  
[Application Health check](http://localhost:60014/health/ready)
[Application metrics URL](http://localhost:60014/metrics) _http://localhost:60014/metrics_  
