...
namespace eID.PJS.UnitTests.Generic;
...
public abstract class BaseTest
{
    ...
    protected static T CreateInterface<T>(object propertyValues)
    {
        ...
        foreach (var property in propertyValues.GetType().GetProperties())
        {
            if (CheckIfAnonymousType(property.PropertyType))
            {
                ...
                var genericMethodInfo = typeof(BaseTest).GetMethod("CreateInterface", BindingFlags.Static | BindingFlags.NonPublic);
    ...
    protected static string BuildMultilineLogMessage(IEnumerable<string> messages)
