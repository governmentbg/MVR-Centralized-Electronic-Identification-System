...
class CertificateStopFragment : BaseFragment<FragmentWithListBinding, CertificateStopViewModel>(),
    CertificateStopAdapter.ClickListener {

    companion object {
        private const val TAG = "CertificateStopFragmentTag"
    }

    override val viewModel: CertificateStopViewModel by viewModels()

    private val adapter: CertificateStopAdapter by inject()

    private val args: CertificateStopFragmentArgs by navArgs()
    ...
    override fun parseArguments() {
        try {
            val identityTypes = buildMap {
                CertificateStopDeviceTypeEnum.entries.map {
    ...
    override fun setupView() {
        binding.toolbar.setTitleText(StringSource(R.string.certificate_stop_screen_title))
    ...
    private fun setAdapterData(data: List<CertificateStopAdapterMarker>) {
