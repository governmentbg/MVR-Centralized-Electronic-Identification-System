...
class EmpowermentLegalDataSource(
    private val legalNumber: String,
    private val sortingModel: EmpowermentSortingModel,
    private val filterModel: EmpowermentFilterModel,
    private val empowermentLegalUiMapper: EmpowermentLegalUiMapper,
    private val getEmpowermentLegalUseCase: GetEmpowermentLegalUseCase,
) : PagingSource<Int, EmpowermentAdapterMarker>() {

    companion object {
        private const val TAG = "EmpowermentFromMeDataSourceTag"
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, EmpowermentAdapterMarker> {
        ...
        try {
            val resultFlow = getEmpowermentLegalUseCase.invoke(
            ...
            return when (finalResult.status) {
                ResultEmittedData.Status.SUCCESS -> {
                    finalResult.model?.let { responseModel ->
                        ...
                        val empowermentsUi =
                            empowermentLegalUiMapper.mapList(empowermentsApiModels)
