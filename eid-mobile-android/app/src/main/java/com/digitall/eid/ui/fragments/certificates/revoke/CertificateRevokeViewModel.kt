package com.digitall.eid.ui.fragments.certificates.revoke
...
class CertificateRevokeViewModel : BaseViewModel() {

    companion object {
        private const val TAG = "CertificateRevokeViewModelTag"
        private const val REVOKED_REASON = "REVOKE_REASON_TYPE"
        private const val REVOKED_STATUS = "REVOKE_EID"
    ...
    private val certificateRevokeUiMapper: CertificateRevokeUiMapper by inject()
    ...
    private val _adapterListLiveData =
        MutableLiveData<List<CertificateRevokeAdapterMarker>>(emptyList())
    ...
    private var certificateRevokeUiModel = CertificateRevokeUiModel()

    private var certificateDetailsModel: CertificateDetailsModel? = null

    private var signingMethod = CertificateRevokeSigningMethodsEnumUi.EVROTRUST
    ...
    fun refreshScreen() {
        ...
        when {
            certificateRevokeUiModel.isValid && certificateDetailsModel != null -> changeStatus()
    ...
    fun onButtonClicked(model: CommonButtonUi) {
        ...
        when (model.elementEnum) {
            CertificateRevokeElementsEnumUi.BUTTON_BACK -> onBackPressed()
            CertificateRevokeElementsEnumUi.BUTTON_CONFIRM -> {
                if (canSubmitForm()) {
                    changeStatus()
                }
            }
    ...
    override fun onSpinnerSelected(model: CommonSpinnerUi) {
        ...
        changeValidationField(model)
        when (model.elementEnum) {
            CertificateRevokeElementsEnumUi.SPINNER_SIGNING_TYPE -> {
                signingMethod =
                    model.selectedValue?.elementEnum as CertificateRevokeSigningMethodsEnumUi
            }
        }
    }
    ...
    private fun changeStatus() {
            ...
            when (element) {
                is CommonEditTextUi -> {
                    when (element.elementEnum) {
                        CertificateRevokeElementsEnumUi.EDIT_TEXT_REASON -> reasonText =
                            element.selectedValue?.trim()

                        CertificateRevokeElementsEnumUi.EDIT_TEXT_FORNAME -> forname =
                            element.selectedValue?.trim()

                        CertificateRevokeElementsEnumUi.EDIT_TEXT_MIDDLENAME -> middlename =
                            element.selectedValue?.trim()

                        CertificateRevokeElementsEnumUi.EDIT_TEXT_SURNAME -> surname =
                            element.selectedValue?.trim()

                        CertificateRevokeElementsEnumUi.EDIT_TEXT_FORNAME_LATIN -> fornameLatin =
                            element.selectedValue?.trim()

                        CertificateRevokeElementsEnumUi.EDIT_TEXT_MIDDLENAME_LATIN -> middlenameLatin =
                            element.selectedValue?.trim()

                        CertificateRevokeElementsEnumUi.EDIT_TEXT_SURNAME_LATIN -> surnameLatin =
                            element.selectedValue?.trim()

                        CertificateRevokeElementsEnumUi.EDIT_TEXT_IDENTIFIER -> citizenIdentifierNumber =
                            element.selectedValue?.trim()

                        CertificateRevokeElementsEnumUi.EDIT_TEXT_DOCUMENT_NUMBER -> documentNumber =
                            element.selectedValue?.trim()

                        CertificateRevokeElementsEnumUi.EDIT_TEXT_ISSUED_FROM -> issuedFrom =
                            element.selectedValue?.trim()

                        CertificateRevokeElementsEnumUi.EDIT_TEXT_CITIZENSHIP -> citizenship =
                            element.selectedValue?.trim()
                    }
                }
                ...
                is CommonSpinnerUi -> {
                    when (element.elementEnum) {
                        CertificateRevokeElementsEnumUi.SPINNER_IDENTIFIER_TYPE -> citizenIdentifierType =
                            element.selectedValue?.serverValue

                        CertificateRevokeElementsEnumUi.SPINNER_DOCUMENT_TYPE -> documentType =
                ...
                is CommonDatePickerUi -> {
                    when (element.elementEnum) {
                        CertificateRevokeElementsEnumUi.DATE_PICKER_ISSUED_ON -> issuedOnDate =
                            element.selectedValue?.toServerDate(dateFormat = ToServerDateFormats.ONLY_DATE)

                        CertificateRevokeElementsEnumUi.DATE_PICKER_VALID_UNTIL -> validUntilDate =
                            element.selectedValue?.toServerDate(dateFormat = ToServerDateFormats.ONLY_DATE)

                        CertificateRevokeElementsEnumUi.DATE_PICKER_DATE_OF_BIRTH -> dateOfBirth =
                            element.selectedValue?.toServerDate(dateFormat = ToServerDateFormats.ONLY_DATE)
                    }
                }
        ...
        val statusRequest = CertificateStatusChangeRequestModel(
            ...
            applicationType = REVOKED_STATUS,
    ...
    private fun fetchInformation() {
        certificateChangeStatusGetInformationUseCaseUseCase.invoke(
            ...
            result.onLoading {
                ...
                certificateRevokeUiModel = certificateRevokeUiModel.copy(
                    userModel = model.userDetails,
                    revokeReasons = model.reasons?.firstOrNull { reason -> reason.name == REVOKED_REASON }?.nomenclatures?.filter { nomenclature ->
                        nomenclature.language == language.type && nomenclature.permittedUser == NomenclaturePermittedUserEnum.PUBLIC
                    }
                )

                setStartElements(certificateRevokeUiModel = certificateRevokeUiModel)
                delay(DELAY_500)
                hideErrorState()
                hideLoader()
    ...
    private fun signApplicationRequest(
        ...
        (when (signingMethod) {
            CertificateRevokeSigningMethodsEnumUi.BORIKA -> certificateApplicationSignWithBoricaUseCase.invoke(
                data = request,
            )

            CertificateRevokeSigningMethodsEnumUi.EVROTRUST -> certificateApplicationSignWithEvrotrustUseCase.invoke(
                data = request,
            )
        }).onEach { result ->
            ...
            }.onFailure { _, title, message, _, errorType ->
            ...
            result.onLoading {
                ...
                when (model) {
                    ApplicationCompletionStatusEnum.COMPLETED -> {
                        showMessage(
                            DialogMessage(
                                message = StringSource(R.string.certificate_revoke_success_message),
                                title = StringSource(R.string.information),
                                positiveButtonText = StringSource(R.string.ok),
                            )
                        )
    ...
    private fun setStartElements(certificateRevokeUiModel: CertificateRevokeUiModel) {
        _adapterListLiveData.postValue(
            certificateRevokeUiMapper.map(
                certificateRevokeUiModel,
    ...
    private fun toPayment(model: ApplicationPaymentModel) {
        ...
        navigateInFlow(
            CertificateRevokeFragmentDirections.toApplicationPaymentFragment(
                model = model
            )
        )
    }
