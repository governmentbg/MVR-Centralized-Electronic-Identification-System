...
import com.digitall.eid.ui.fragments.applications.show.all.ApplicationsFragment
...
class CertificatesFragment :
    BaseFragment<FragmentWithListAndSortBinding, CertificatesViewModel>(),
    CertificatesAdapter.ClickListener {

    companion object {
        private const val TAG = "CertificatesFragmentTag"
        private const val REFRESH_CERTIFICATES_REQUEST_KEY = "REFRESH_CERTIFICATES_REQUEST_KEY"
        private const val REFRESH_CERTIFICATES_KEY = "REFRESH_CERTIFICATES_KEY"
    }

    override fun getViewBinding() =
        FragmentWithListAndSortBinding.inflate(layoutInflater)

    override val viewModel: CertificatesViewModel by viewModel()
    private val adapter: CertificatesAdapter by inject()
    private val adapterDataObserver: RecyclerViewAdapterDataObserver by inject()
    private val args: CertificatesFragmentArgs by navArgs()
    ...
    override fun parseArguments() {
        try {
            ...
            logDebug("parseArguments id is null", TAG)
        }
    }
    ...
    override fun setupControls() {
        ...
        setFragmentResultListener(FILTER_MODEL_REQUEST_KEY) { _, bundle ->
            val filterModel = bundle.getParcelableCompat<CertificatesFilterModel>(FILTER_MODEL_KEY)
            filterModel?.let {
                viewModel.updateFilteringModel(it)
            }
        }

        setFragmentResultListener(REFRESH_CERTIFICATES_REQUEST_KEY) { _, bundle ->
            val shouldRefresh = bundle.getBoolean(REFRESH_CERTIFICATES_KEY, false)
            if (shouldRefresh) {
                adapter.refresh()
            }
        }
    ...
    override fun onOpenClicked(model: CertificateUi) {
        logDebug("onOpenClicked", TAG)
        viewModel.onOpenClicked(model)
    }

    override fun onSpinnerClicked(model: CertificateUi, anchor: View) {
        logDebug("onOpenClicked", TAG)
