...
import com.digitall.eid.ui.fragments.empowerment.to.me.all.list.EmpowermentToMeDataSource
...
class EmpowermentToMeViewModel : BaseEmpowermentViewModel() {

    companion object {
        private const val TAG = "EmpowermentToMeViewModelTag"
        private const val PAGE_SIZE = 20
    }

    private val empowermentToMeUiMapper: EmpowermentToMeUiMapper by inject()
    private val getEmpowermentToMeUseCase: GetEmpowermentToMeUseCase by inject()
    ...
    override val empowermentsPagingDataFlow: Flow<PagingData<EmpowermentAdapterMarker>> = combine(
        ...
        .flatMapLatest { (filter, sortCriteria) ->
            Pager(
                ...
                pagingSourceFactory = {
                    EmpowermentToMeDataSource(
                        sortingModel = sortCriteria,
                        filterModel = filter,
                        empowermentToMeUiMapper = empowermentToMeUiMapper,
                        getEmpowermentToMeUseCase = getEmpowermentToMeUseCase,
                    )
                }
            ).flow
    ...
    override fun toFilter() {
        ...
        navigateInFlow(
            EmpowermentToMeFragmentDirections.toEmpowermentToMeFilterFragment(
                model = filteringModel.value
            )
        )
    }

    fun toDetails(model: EmpowermentToMeUi) {
        logDebug("toDetails", TAG)
        navigateInFlow(
            EmpowermentToMeFragmentDirections.toEmpowermentToMeDetailsFragment(
    ...
    override fun toCancel(model: CommonSpinnerUi) {
        ...
        navigateInFlow(
            EmpowermentToMeFragmentDirections.toEmpowermentToMeCancelFragment(
                model = empowermentItem
            )
        )
    }

    override fun toInquiry() = Unit

    override fun onSortingSpinnerClicked() {
        logDebug("onSortingSpinnerClicked", TAG)
        viewModelScope.launchWithDispatcher {
            ...
            val data = CommonSpinnerUi(
                ...
                list = buildList {
                    ...
                    identifier = EmpowermentSortingModelUi(
                        sortBy = EmpowermentSortingByEnum.AUTHORIZER,
                        sortDirection = EmpowermentSortingDirectionEnum.ASC
                    )
                    add(
                        CommonSpinnerMenuItemUi(
