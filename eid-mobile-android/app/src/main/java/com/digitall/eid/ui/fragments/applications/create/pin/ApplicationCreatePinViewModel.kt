...
class ApplicationCreatePinViewModel : BaseCreatePinViewModel() {

    companion object {
        private const val TAG = "ApplicationCreatePinViewModelTag"
    }
    ...
    fun onCreateClicked() {
        ...
        applicationCreateConfirmWithEIDUseCase.invoke(
            ...
            result.onLoading {
                logDebug("applicationConfirmWithEIDUseCase onLoading", TAG)
                showLoader(message = it)
            }.onSuccess { _, _, _ ->
                logDebug("applicationConfirmWithEIDUseCase onSuccess", TAG)
                val applicationInfo = preferences.readApplicationInfo()
                if (applicationInfo == null) {
                    logError("applicationConfirmWithEIDUseCase applicationInfo == null", TAG)
                    return@onEach
                }
                ...
                logError("applicationConfirmWithEIDUseCase onFailure", message, TAG)
                hideLoader()
                when (errorType) {
