 ...
 **/
package com.digitall.eid.mappers.empowerment.from.me.details
...
class EmpowermentFromMeDetailsUiMapper :
    ...
    override fun map(
        ...
        return with(from) {
            ...
            buildList {
                ...
                if (!serviceId.isNullOrEmpty() && !serviceName.isNullOrEmpty()) {
                    add(
                        CommonSimpleTextUi(
                    ...
                    when (status.first) {
                        EmpowermentDetailsStatementsElementsUi.COLLECTING_AUTHORIZER_SIGNATURES -> {
                            empowermentSignatures?.forEach { empowermentSignature ->
                                val signerModel = authorizerUids?.first { element -> element.uid == empowermentSignature.signerUid }
