...
class SetNotSelectedParentNotificationUseCase : BaseUseCase {

    companion object {
        private const val TAG = "SetNotSelectedParentNotificationUseCaseTag"
    ...
    fun invoke(id: String): Flow<ResultEmittedData<Unit>> = flow {
        ...
        errorState = false
        val eventsID = notificationsDatabaseRepository.getNotificationById(id)
            .events
            ?.filter { it.isMandatory == false }
            ?.map { it.id }?.toSet() ?: emptySet()
        val current = notificationsDatabaseRepository.getNotSelectedNotifications()
            .toMutableSet()
        if (current.any { it in eventsID }) {
            logDebug("invoke any", TAG)
            current.removeAll(eventsID)
        } else {
            logDebug("invoke not any", TAG)
            current.addAll(eventsID)
        }
        logDebug("invoke events size: ${eventsID.size} current size: ${current.size}", TAG)
