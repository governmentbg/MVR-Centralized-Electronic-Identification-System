...
class GetEmpowermentProvidersUseCase : BaseUseCase {
    ...
    private val data = mutableListOf<EmpowermentProviderModel>()

    fun invoke(): Flow<ResultEmittedData<List<EmpowermentProviderModel>>> = flow {
        logDebug("invoke", TAG)
        pageIndex = 1
        data.clear()
        getNext(this@flow)
    ...
    private suspend fun getNext(
        flow: FlowCollector<ResultEmittedData<List<EmpowermentProviderModel>>>
    ) {
        logDebug("getNext", TAG)
        empowermentCreateNetworkRepository.getEmpowermentProviders(
            pageSize = PAGE_SIZE,
            pageIndex = pageIndex,
        ).onEach { result ->
            result.onLoading {
