...
import bg.bulsi.mvr.raeicei.gateway.api.v1.EidManagerFrontOfficeApiDelegate;
...
public class EidManagerFrontOfficeService implements EidManagerFrontOfficeApiDelegate {
    ...
    @Override
    public Mono<List<EidManagerFrontOfficeDTO>> getAllEidManagerFrontOffices(UUID eidManagerId, ServerWebExchange exchange) {
    ...
    @Override
    public Mono<List<EidManagerFrontOfficeDTO>> getAllEidManagerOfficesByRegion(String region, LocalDate from, LocalDate to, ServerWebExchange exchange) {
        Mono<List> result = eventSender.send(exchange, new OfficesByRegionDTO(region, from, to), AuditEventType.GET_ALL_EID_MANAGER_OFFICES_BY_REGION, List.class);
        return result.map(e -> new ArrayList<EidManagerFrontOfficeDTO>(e));
    }
}
