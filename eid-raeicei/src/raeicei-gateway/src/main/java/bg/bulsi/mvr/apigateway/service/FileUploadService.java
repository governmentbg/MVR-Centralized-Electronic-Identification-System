...
import bg.bulsi.mvr.raeicei.gateway.api.v1.FileUploadApiDelegate;
...
//    }
    ...
    @Override
    public Mono<DocumentResponseDTO> downloadTechnicalCheckProtocol(ServerWebExchange exchange) {
        return eventSender.send(exchange, new HashMap<>(), AuditEventType.GET_TECHNICAL_CHECK_PROTOCOL, DocumentResponseDTO.class);
    }
    ...
    public Mono<List<DocumentDTO>> uploadDocumentForApplication(UUID id, List<DocumentDTO> documents, ServerWebExchange exchange) {
        ...
        return eventSender.send(exchange, messageBody, AuditEventType.CREATE_DOCUMENT, List.class).map(e -> (List<DocumentDTO>) e);
    }

    @Override
    public Mono<List<DocumentDTO>> uploadDocumentForEidManager(List<DocumentDTO> documents, ServerWebExchange exchange) {
        return eventSender.send(exchange, documents, AuditEventType.CREATE_DOCUMENT, List.class).map(e -> (List<DocumentDTO>) e);
    }
}
