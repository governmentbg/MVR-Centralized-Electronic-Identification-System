...
export class ApplicationsFilterComponent implements OnInit {
    constructor(
        ...
        this.languageChangeSubscription = translateService.langChanges$.subscribe(() => {
            this.statuses = [
                ...
                {
                    name: this.translateService.translate('modules.eidManagement.applicationStatus.txtPaymentExpired'),
                    code: ApplicationStatus.PAYMENT_EXPIRED,
                    icon: 'e644',
                },
                {
                    name: this.translateService.translate('modules.eidManagement.applicationStatus.txtSubmitted'),
                    code: ApplicationStatus.SUBMITTED,
                    icon: 'e14b',
                },
                {
                    name: this.translateService.translate('modules.eidManagement.applicationStatus.txtApproved'),
                    code: ApplicationStatus.APPROVED,
                    icon: 'e877',
                },
                {
                    name: this.translateService.translate('modules.eidManagement.applicationStatus.txtCompleted'),
                    code: ApplicationStatus.COMPLETED,
                    icon: 'e877',
                },
                {
                    name: this.translateService.translate('modules.eidManagement.applicationStatus.txtSigned'),
                    code: ApplicationStatus.SIGNED,
                    icon: 'e877',
                },
                {
                    name: this.translateService.translate('modules.eidManagement.applicationStatus.txtPendingPayment'),
                    code: ApplicationStatus.PENDING_PAYMENT,
                    icon: 'e5c9',
                },
                {
                    name: this.translateService.translate('modules.eidManagement.applicationStatus.txtPaid'),
    ...
    prepareFilteredValueLabel(filteredValue: any): string {
        if (filteredValue.key === 'createDate') {
    ...
    extractValues(): any {
        ...
        if (extractedValues && extractedValues.createDate) {
            const utcDate = this.getUTCDateRange(this.formatDate(extractedValues.createDate));

            extractedValues.createdDateFrom = utcDate.startUTC;
            extractedValues.createdDateTo = utcDate.endUTC;
            delete extractedValues.createDate;
        }

        if (extractedValues && extractedValues.status === 'SIGNED') {
            extractedValues.status = ['SIGNED', 'GENERATED_CERTIFICATE', 'CERTIFICATE_STORED'];
        }

        return extractedValues;
    }

    getUTCDateRange(localDate: string) {
